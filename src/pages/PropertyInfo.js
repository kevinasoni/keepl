...
const emptyForm = { propertyType: '', address: '', marketValue: '' };
...
const PropertyInfo = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.ok && data.data?.propertyInfo) { setRecords(data.data.propertyInfo); setShowForm(false); }
  ...
  const saveData = async (updated) => {
    const res = await fetch(`${API_URL}/api/user-data`, {
      ...
      body: JSON.stringify({ propertyInfo: updated }),
    });
    return res.ok;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (await saveData(updated)) {
      toast.success(editingIndex !== null ? 'Updated!' : 'Property saved!');
    ...
    <Container>
      ...
      {showForm && (
        <form onSubmit={handleSubmit}>
          ...
          <Input value={formData.propertyType} onChange={e => setFormData(p => ({ ...p, propertyType: e.target.value }))} placeholder="Apartment, Land, Commercial" required />
          <Label>Address</Label>
          <Input value={formData.address} onChange={e => setFormData(p => ({ ...p, address: e.target.value }))} placeholder="Property address" required />
          <Label>Market Value (INR)</Label>
          <Input type="number" value={formData.marketValue} onChange={e => setFormData(p => ({ ...p, marketValue: e.target.value }))} placeholder="Current market value" required />
          <Button type="submit" disabled={loading}>{loading ? 'Saving...' : editingIndex !== null ? 'Update' : 'Save Property'}</Button>
      ...
      {records.map((r, i) => (
        <Card key={i}>
          ...
          <p><strong>Value:</strong> ₹{r.marketValue}</p>
